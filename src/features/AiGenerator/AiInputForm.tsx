...
import inserIcon from './Asset/insert.svg'
...
const AiInputForm: React.FC<AiInputFormProps> = ({ prompt, setPrompt, response="" , insetFunction }) => {
  ...
  return (
    <form className="flex flex-col max-w-lg mx-auto" onSubmit={submitFunction} >
     ...
     { (response !== "") &&  
      ...
      >
       <img src={inserIcon}  className="inline w-[1.5rem] h-[1.5rem] " alt="icon" />
        ...
        >
          <img src={response !== "" ? RegeneRateIcon : GenerateIcon} className="inline w-[1.5rem] h-[1.5rem]" alt="icon" />
          <span className='text-[1.5rem]'>{response !== "" ? "Regenerate" : "Generate"}</span>
        </button>
